...
namespace ast {
    ...
    using Value = long; // int, if you prefer
...
//    std::cout << "Output file name: " << argv[2] << std::endl;
//    
    std::cout << "Symbol '*' at your console means that one expression was processed and saved to file" << std::endl;
    
    std::ifstream ifs("/Users/g_alik/valuator.xml");
    std::ofstream ofs;
    ofs.open ("/Users/g_alik/valuator_.xml", std::ofstream::trunc | std::ofstream::out);
    ...
    try {
        ...
        if (ok)
        {
            ...
            ofs << "<expressions>" << std::endl;
            for (auto& expr : parsed)
                if (get_id(expr)) {
                    std::cout << "*";
                    ofs << "\t<result id=\"" << *get_id(expr) << "\">" << machine.evaluate(expr) << "</result>" << std::endl;
                }
            ofs << "</expressions>" << std::endl;
        }
    } catch(qi::expectation_failure<It> const& e) {
        std::cout << "Expected " << e.what_ << " at '" << std::string(e.first,e.last) << "'" << std::endl;
    }
    std::cout << std::endl << "Done with parsing, result is saved to output file" << std::endl << "Exit" << std::endl;
    ofs.close();
}